...
public class SecurityUtil {
    ...
    public static String hashSHA256(String password) {
        return hashSHA256(password, null);
    }
     ...
     *
     * @param password La cadena de texto (como una contraseña) que se va a procesar.
     * @param encodedhash Arreglo opcional de bytes.
     * @return El hash SHA-256 resultante representado como una cadena en formato hexadecimal.
    ...
    public static String hashSHA256(String password, byte[] encodedhash) {
        try {
            ...
            byte[] bytesGenerados = digest.digest(password.getBytes(java.nio.charset.StandardCharsets.UTF_8));
            
            byte[] arregloAUsar = (encodedhash != null && encodedhash.length > 0) ? encodedhash : bytesGenerados;

            StringBuilder hexString = new StringBuilder(2 * arregloAUsar.length);
            for (byte b : arregloAUsar) {
